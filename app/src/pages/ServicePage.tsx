...
export default function ServicePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchService(id),
      supabase.from('workshop_reviews').select('*, users(name, phone)').eq('service_id', id).order('created_at', { ascending: false }),
    ]).then(([svc, rev]) => {
      setService(svc)
      setReviews(rev.data ?? [])
      if (user) {
        const mine = rev.data?.find(r => r.user_id === user.id)
        if (mine) { setMyReview(mine); setStars(mine.rating); setComment(mine.comment ?? '') }
      }
      setLoading(false)
    })
  }, [id, user])
  ...
  async function submitReview() {
    ...
    if (!error) {
      // Refresh reviews
      const { data } = await supabase.from('workshop_reviews').select('*, users(name, phone)').eq('service_id', id!).order('created_at', { ascending: false })
      setReviews(data ?? [])
      // Refresh rating in service
      const { data: avg } = await supabase.from('services').select('rating').eq('id', id!).single()
      setService((s: any) => ({ ...s, rating: avg?.rating }))
      setMyReview({ rating: stars, comment: comment.trim() || null })
      setSubmitted(true)
    }
    setSubmitting(false)
  }
          ...
          <div>
              ...
              <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', marginBottom: 20 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', fontSize: 14, color: 'var(--text-3)' }}>
                    <span style={{ display: 'flex', gap: 5, alignItems: 'center' }}><MapPin size={13}/> {service.city}</span>
                    {service.opening_hours && (
                      <span style={{ display: 'flex', gap: 5, alignItems: 'center' }}><Clock size={13}/> {service.opening_hours}</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div style={{ background: '#fff', borderRadius: 20, padding: '24px', border: '1px solid var(--gray-200)', boxShadow: 'var(--shadow-sm)' }}>
              <h2 style={{ fontSize: 17, fontWeight: 800, marginBottom: 20 }}>
                التقييمات
                {reviews.length > 0 && <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-4)', marginRight: 8 }}>({reviews.length})</span>}
              </h2>
              ...
              {submitted || myReview ? (
                ...
                <div style={{ background: 'var(--gray-50, #F9FAFB)', borderRadius: 14, padding: '18px', marginBottom: 24, border: '1px solid var(--gray-200)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
                    ...
                    {(hover || stars) > 0 && (
                      <span style={{ fontSize: 13, color: 'var(--text-3)', alignSelf: 'center', marginRight: 4 }}>
                        {['','ضعيف','مقبول','جيد','جيد جداً','ممتاز'][hover || stars]}
                      </span>
                    )}
                  </div>

                  <textarea className="input" rows={2} placeholder="اكتب تعليقك (اختياري)..."
                    value={comment} onChange={e => setComment(e.target.value)}
                    style={{ resize: 'none', marginBottom: 12, fontSize: 14 }}/>

                  <button className="btn btn-yellow" onClick={submitReview} disabled={!stars || submitting}
                    style={{ fontSize: 14, gap: 7 }}>
                    {submitting ? <Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }}/> : <Star size={14}/>}
                    أرسل التقييم
                  </button>
                </div>
              ) : (
                <div style={{ background: 'var(--gray-100)', borderRadius: 12, padding: '14px 18px', marginBottom: 20, fontSize: 13, color: 'var(--text-3)' }}>
                  <Link to="/login" style={{ color: 'var(--blue)', fontWeight: 700 }}>سجّل دخولك</Link> لتقييم هذه الورشة
                </div>
              )}

              {/* Reviews list */}
              {reviews.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '24px 0', color: 'var(--text-4)', fontSize: 14 }}>
                  لا يوجد تقييمات بعد — كن أول من يقيّم!
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {reviews.map(r => (
                    <div key={r.id} style={{ paddingBottom: 14, borderBottom: '1px solid var(--gray-100)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--yellow)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 13 }}>
                            {(r.users?.name ?? r.users?.phone ?? '?')[0]}
                          </div>
                          <span style={{ fontSize: 14, fontWeight: 600 }}>{r.users?.name ?? 'مستخدم كارنا'}</span>
                        </div>
                      ...
                      {r.comment && <p style={{ fontSize: 14, color: 'var(--text-2)', lineHeight: 1.7, margin: '0 40px' }}>{r.comment}</p>}
                      <div style={{ fontSize: 11, color: 'var(--text-4)', marginTop: 4, marginRight: 40 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 20, padding: '24px', border: '1px solid var(--gray-200)', boxShadow: 'var(--shadow-md)' }}>
              ...
              {!service.phone && !service.whatsapp && (
                <div style={{ textAlign: 'center', padding: '12px 0', color: 'var(--text-4)', fontSize: 13 }}>
                  تواصل عبر <Link to="/contact" style={{ color: 'var(--blue)' }}>صفحة التواصل</Link>
                </div>
              )}
              ...
              {service.whatsapp && (
                <a href={`https://wa.me/${service.whatsapp}`} target="_blank" rel="noopener noreferrer"
                  className="btn" style={{ width: '100%', justifyContent: 'center', fontSize: 15, background: '#25D366', color: '#fff', border: 'none', gap: 8 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {service.address && (
                  <div style={{ display: 'flex', gap: 10, fontSize: 13, color: 'var(--text-2)' }}>
                    <MapPin size={14} style={{ flexShrink: 0, marginTop: 1, color: 'var(--text-4)' }}/>{service.address}
                  </div>
                )}
                {service.opening_hours && (
                  <div style={{ display: 'flex', gap: 10, fontSize: 13, color: 'var(--text-2)' }}>
                    <Clock size={14} style={{ flexShrink: 0, marginTop: 1, color: 'var(--text-4)' }}/>{service.opening_hours}
                  </div>
                )}
              </div>
