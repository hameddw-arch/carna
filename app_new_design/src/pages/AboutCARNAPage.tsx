...
export default function AboutCARNAPage() {
  ...
  const stats = [
    { label: 'سيارة مدرجة', value: '+10,000', iconName: 'directions_car' },
    { label: 'ورشة صيانة', value: '+500', iconName: 'construction' },
    { label: 'مدينة سورية', value: '+20', iconName: 'location_city' }
  ]

  const values = [
    { iconName: 'verified', title: 'الشفافية', desc: 'نسعى لترسيخ الصدق في التعامل من خلال أنظمة فحص وتوثيق دقيقة' },
    { iconName: 'lightning_bolt', title: 'السرعة', desc: 'منصة تقنية متطورة تتيح لك الوصول إلى ما تحتاجه في ثوانٍ معدودة' },
    { iconName: 'handshake', title: 'الثقة', desc: 'بناء مجتمع آمن يجمع بين أفضل التجار والورشات الموثوقة' }
  ]
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-surface dark:bg-background text-on-surface dark:text-slate-50 overflow-x-hidden transition-colors duration-300">
      ...
      {/* Our Story Section */}
      <section ref={storyRef} className="py-16 md:py-24 px-4 md:px-8 dark:bg-background">
      ...
      {/* Mission Section */}
      <section ref={missionRef} className="bg-surface-container dark:bg-surface py-16 md:py-24 px-4 md:px-8">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className={`grid md:grid-cols-3 gap-8 ${missionInView ? 'animate-in' : 'opacity-0'}`}>
            {values.map((value, idx) => (
              <div
                key={idx}
                className="bg-surface-white dark:bg-surface-white p-8 rounded-xl border border-border-light dark:border-border-light hover:shadow-lg dark:hover:shadow-xl transition-all duration-300"
                style={{ animationDelay: `${idx * 100}ms` }}
              >
                <span className="material-symbols-outlined text-5xl text-primary dark:text-yellow-400 flex justify-center mb-4">{value.iconName}</span>
                <h3 className="font-bold text-lg dark:text-white mb-3">{value.title}</h3>
                <p className="text-on-surface-variant dark:text-on-surface-variant">{value.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Services Section */}
      <section ref={servicesRef} className="py-16 md:py-24 px-4 md:px-8 dark:bg-background">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="space-y-8">
            {services.map((service, idx) => (
              <div
                key={idx}
                className={`flex flex-col ${idx % 2 === 1 ? 'md:flex-row-reverse' : 'md:flex-row'} gap-8 items-center bg-surface-white dark:bg-surface border border-border-light dark:border-border-light p-8 rounded-xl ${
                  servicesInView ? 'animate-in' : 'opacity-0'
                }`}
                style={{ animationDelay: `${idx * 150}ms` }}
              >
                <div className="flex-shrink-0 md:w-1/4 flex justify-center">
                  <span className="material-symbols-outlined text-7xl text-primary dark:text-yellow-400">{service.iconName}</span>
                </div>
                <div className="flex-1">
                  <h3 className="text-2xl font-bold dark:text-white mb-4">{service.title}</h3>
                  <p className="text-on-surface-variant dark:text-on-surface-variant text-lg">{service.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Statistics Section */}
      <section ref={statsRef} className="bg-inverse-surface dark:bg-background text-white py-16 md:py-24 px-4 md:px-8">
        <div className="max-w-6xl mx-auto">
          <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 text-center ${statsInView ? 'animate-in' : 'opacity-0'}`}>
            {stats.map((stat, idx) => (
              <div key={idx} style={{ animationDelay: `${idx * 100}ms` }}>
                <span className="material-symbols-outlined text-5xl text-yellow-400 flex justify-center mb-4">{stat.iconName}</span>
      ...
      {/* CTA Section */}
      <section className="py-16 md:py-24 px-4 md:px-8 dark:bg-background">
      ...
      {/* Footer */}
      <footer className="bg-surface-container dark:bg-background border-t border-border-light dark:border-border-light mt-auto py-12 px-4 md:px-8">
        <div className="max-w-6xl mx-auto">
          <div className="hidden md:flex gap-8 justify-center mb-8 pb-8 border-b border-border-light dark:border-border-light">
            <a href="/" className="text-on-surface-variant dark:text-on-surface-variant hover:text-primary dark:hover:text-yellow-400 transition-colors">عن كارنا</a>
            <a href="/contact" className="text-on-surface-variant dark:text-on-surface-variant hover:text-primary dark:hover:text-yellow-400 transition-colors">اتصل بنا</a>
            <a href="/privacy" className="text-on-surface-variant dark:text-on-surface-variant hover:text-primary dark:hover:text-yellow-400 transition-colors">سياسة الخصوصية</a>
            <a href="/terms" className="text-on-surface-variant dark:text-on-surface-variant hover:text-primary dark:hover:text-yellow-400 transition-colors">الشروط والأحكام</a>
          </div>
          <div className="text-center">
            <div className="font-bold text-primary dark:text-yellow-400 text-lg mb-2">CARNA</div>
            <p className="text-on-surface-variant dark:text-on-surface-variant">© 2026 كارنا - كافة الحقوق محفوظة</p>
          </div>
        </div>
      </footer>
    </div>
